-- seed.sql
-- Real seed data for the employees and payroll_runs tables (Azhar).
--
-- Source of truth: this is the SAME 10 employees that appear in all three
-- current front-end copies (frontend/data/employee_info.json,
-- frontend/vue-app/data.js, and the employeeInfo array in dashboard.js) —
-- they already agree on names/positions/departments/salaries/contacts, so
-- there is no real conflict to resolve for the employees table itself.
-- What's replaced here is just the placeholder 3-employee sample data that
-- was previously in this file.
--
-- Run after schema: mysql -u root -p moderntech < db/moderntech.sql
--                    mysql -u root -p moderntech < db/seed.sql

USE moderntech;

-- Clear placeholder/previous seed data (children first, FK order)
DELETE FROM payroll_runs;
DELETE FROM users WHERE employee_id IS NOT NULL;
DELETE FROM employees;
DELETE FROM departments;

-- Reset auto-increment counters so employee_id lines up with the
-- employeeId values used throughout the front-end (1–10)
ALTER TABLE employees AUTO_INCREMENT = 1;
ALTER TABLE departments AUTO_INCREMENT = 1;
ALTER TABLE payroll_runs AUTO_INCREMENT = 1;

-- DEPARTMENTS ---------------------------------------------------------
-- The 9 distinct department names used across employee_info.json /
...
-- EMPLOYEES -------------------------------------------------------------
-- employee_id will auto-assign 1–10 in this insert order, matching the
-- employeeId used in employee_info.json / data.js / dashboard.js.
-- hire_date is inferred from the "Joined in YYYY" text in
-- employment_history (Jan 1 of that year) since the front-end never had a
-- real date field — flag this assumption to the team if a precise date
...
-- PAYROLL_RUNS --------------------------------------------------------------
-- One pay period (July 2025, matching the July 2025 dates already used as
-- dummy data in attendance) generated by actually calling
-- services/payrollCalc.service.js — NOT copied from dashboard.js's old
-- hardcoded `payrollData`/`finalSalary` array.
--
-- IMPORTANT — flag this to the team, don't just merge quietly:
-- dashboard.js's old payrollData treated `leaveDeductions` as a raw HOURS
-- figure and subtracted it more or less directly from salary with no tax
-- applied (e.g. employee 1: 70000 -> 69500). The real calc service treats
-- leave_deductions as a CURRENCY amount and applies an 18% flat tax on top
-- (a placeholder rate — see TAX_RATE in payrollCalc.service.js). To keep
-- the numbers meaningful I converted each employee's leave hours to a
-- dollar amount at their hourly rate (salary / 160) before calling the
-- service. Net result: these net_pay figures will NOT match the old
-- dashboard mock numbers, and that mismatch is expected — it's the exact
-- "decide with the team" item called out in James's dashboard section of
-- the implementation plan (payroll-trend / KPI formulas).
